...
## 9. Auth & Backend — Real Working 2FA (Supabase)

Use **Supabase** (free tier) for both the database and authentication — it has built-in email OTP sign-in, so no custom email-sending or code-verification logic needs to be written from scratch.

**Setup:**
1. Create a free Supabase project at supabase.com
2. Use Supabase Auth's email OTP method: `supabase.auth.signInWithOtp({ email })` — this sends a 6-digit code to the given email address automatically
3. On the code-entry screen, call `supabase.auth.verifyOtp({ email, token, type: 'email' })` to confirm login
4. Use Supabase's Postgres database (free tier) to store the `Student`, `CCA`, and `Event` tables from Section 2 — Supabase auto-generates a REST API for these tables, so Antigravity doesn't need to build a separate backend server

**For the demo:**
- Use each team member's real Outlook email as their login email during testing/demo — the code will genuinely arrive in Outlook, satisfying "sign in with NP ID, get a 2FA code in Outlook" without needing real NP system access
- In the journal, note that in a real deployment this would integrate with NP's actual student email system; for the prototype, Supabase Auth demonstrates the same 2FA flow with a real email provider

**Fallback if Supabase setup has issues close to deadline:** revert to a mocked 2FA screen (static/generated code shown on-screen, no real email sent) — still fully acceptable per the brief's clickable-prototype standard, just less impressive in the demo.

**Ties to Defence pillar:** email OTP is a concrete control mitigating "unauthorised access to student account via stolen/shared password" — asset: student account, threat: credential compromise, vulnerability: single-factor login, CIA: Confidentiality, control: OTP second factor via email.

## 10. Notes for Antigravity / Claude Code
...
- Use Supabase for auth + database as described in Section 9, rather than building custom email/OTP logic.
